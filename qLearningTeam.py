...
def createTeam(firstIndex, secondIndex, isRed,
                first = 'ApproximateQAgent', second = 'DefensiveReflexAgent', **args):
    """
...
class ApproximateQAgent(OffensiveReflexAgent):
    ...
    def chooseAction(self, state):
        ...
        if len(legalActions):
            if util.flipCoin(self.epsilon) and self.isTraining():
                ...
                '''
        self.epsilon = max(.01,self.epsilon-.0001)
    ...
    def final(self, state):
        ...
        if self.isTraining() and DEBUG_ONCE_PER:
            print "END WEIGHTS"
            print self.weights
